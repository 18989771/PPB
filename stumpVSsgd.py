...
class AdaBoostDecisionStump:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            ...
            EPS = 1e-10
            if error >= 0.5:
                print(f"Stopping early at iteration {i+1} due to error >= 0.5 ({error:.4f})")
                break
            if error == 0:
                print(f"Stopping early at iteration {i+1} due to perfect fit")
                break
            ...
            print(f"Iteration {i+1}: Weak learner error = {error:.4f}, alpha = {alpha:.4f}, training error = {err:.4f}")
...
# Train models
ada = AdaBoostDecisionStump(n_estimators=100)
